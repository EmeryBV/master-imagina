...
int main(int argc, char const *argv[])
{
	...
	else if (is_ppm_header(header))
	{
		...
		PPM image_out = reduced_by_2(image_in);

		// RGB_to_YCbCr(image_out);
		// image_out = reduced_by_2(image_out);
		// YCbCr_to_RGB(image_out);

		// cerr << "Transformation spacial (RGB -> YCbCr)...\n";
		// RGB_to_YCbCr(image_out);

		// PGM Y = image_out.green();
		// PGM Cb = reduced_by_2(image_out.blue());
		// PGM Cr = reduced_by_2(image_out.red());
		...
		cerr << "Taille originale    : " << image_in.size() * 3  << " octets\n";
		cerr << "Taille compressée   : " << image_out.size() * 3 << " octets\n";
		cerr << "Taux de compression : " << (double)image_in.size() / image_out.size() << '\n';

...
template<typename T>
T clamp(const T& value, const T& min = 0, const T& max = 255)
{
	assert(min < max);
...
void YCbCr_to_RGB(PPM& img_ycbcr)
{
	...
	{
		double R = double(px.g) + 1.402    * (double(px.r) - 128);
		double G = double(px.g) - 0.344136 * (double(px.b) - 128) - 0.714136 * (double(px.r) - 128);
		double B = double(px.g) + 1.772    * (double(px.b) - 128);

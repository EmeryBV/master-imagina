...
Color::Color(OCTET r, OCTET g, OCTET b) : r(r), g(g), b(b) {}

Color::Color(const OCTET rgb[3]) : r(rgb[0]), g(rgb[1]), b(rgb[2]) {}
...
Color& Color::operator[](size_t n)
{
	return 
}

const Color& Color::operator[](size_t n) const
{

}

Color& Color::operator()(size_t row, size_t column)
{

}

const Color& Color::operator()(size_t row, size_t column) const
{

}

OCTET& ImagePPM::red(size_t n) {
	return this->m_red[n];
}
...
const OCTET& ImagePPM::red(size_t n) const{
	return this->m_red[n];
}
...
OCTET& ImagePPM::green(size_t n) {
	return this->m_green[n];
}
...
const OCTET& ImagePPM::green(size_t n) const{
	return this->m_green[n];
}
...
OCTET& ImagePPM::blue(size_t n) {
	return this->m_blue[n];
}
...
const OCTET& ImagePPM::blue(size_t n) const{
	return this->m_blue[n];
}
...
void ImagePPM::read(std::istream& is) {
	...
	is.read((char*)m_data.data(), m_data.size() * 3);
...
*/
	os.write((char*)m_data.data(), m_data.size() * 3);

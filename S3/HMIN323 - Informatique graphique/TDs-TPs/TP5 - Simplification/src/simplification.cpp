...
struct Mesh 
{
    ...
    std::vector<INDICE_TYPE> vertexes_indices;
...
int main( void )
{
    ...
    Grid3 grid(bb_min, bb_max, 64);

    std::vector<glm::vec3> lines_vertices = grid.get_vertices();
...
/*
    // buffer de ligne pour le debbugage
    GLuint linebuffer;
    glGenBuffers(1, &linebuffer);
    glBindBuffer(GL_ARRAY_BUFFER, linebuffer);
    glBufferData(GL_ARRAY_BUFFER, lines_vertices.size() * sizeof(glm::vec3), &lines_vertices[0], GL_STATIC_DRAW);
...
// maverick.inria.fr/stages
    ...
    do{
        ...
        // grid.draw();
         ...
         float point[3] = {90, 0, 0};

         glBegin(GL_LINES);
            glVertex3f(bb_min.x, bb_min.x, bb_min.z);
            glVertex3f(bb_max.x, bb_max.x, bb_max.z);
         glEnd();

        // draw_line({-1, 1, 0}, {1, -1, 0});
